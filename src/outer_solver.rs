...
use crate::{BoolExp, Conjunction, Disjunction, Exp, Solver, Sort, HasSort};
use alloc::vec::Vec;
...
impl FnSort {
    ...
    pub fn try_new<E>(args: impl Iterator<Item=Result<Sort, E>>, ret: Sort) -> Result<Self, E> {
        Ok(FnSort{args: args.collect::<Result<_, E>>()?, ret})
    }
...
///    solver.start_exp(x_sym, None, Opt);
///    solver.end_exp(false).unwrap(); // x
///    solver.end_exp(true).unwrap(); // (f x x)
/// });
/// ```
///
...
/// solver.start_exp(x_sym, None, Opt);
/// solver.end_exp(false).unwrap(); // x
/// solver.end_exp(true).unwrap(); // (f (f true false) x)
/// ```
...
impl OuterSolver {
    ...
    pub fn with_defined<O>(&mut self, symbol: Symbol, value: Exp, f: impl FnOnce(&mut Self) -> O) -> O {
    ...
    fn defined_symbols(&self) -> impl Iterator<Item=Symbol> + '_ {
        self.bound.keys().copied().filter(|&k| k != TRUE_SYM && k != FALSE_SYM)
    }
    ...
    pub fn start_exp(&mut self, f: Symbol, expected: Option<Sort>, ctx: StartExpCtx) {
        let ctx = match (ctx, self.stack.last()) {
            ...
            (StartExpCtx::Opt, Some(x)) => self.child_context(x.ctx, f),
            (ctx, last) => {
                let not = if last.is_some() {""} else { " not" };
                panic!("Invalid ctx {ctx:?} when{not} building existing expression")
            }
        };
        self.stack.push(Frame{ctx, f, expected, stack_len: self.exp_stack.len() as u32})
    }
    ...
    fn end_exp_inner(
        ...
        let mut base_children = self.exp_stack[stack_len as usize..].iter().copied().enumerate().map(IndexExp);
        let children = &mut base_children;
        let res: Exp = match f {
            AND_SYM => if matches!(ctx, ExprContext::Assert {negate: false}) {
                children.map(|x| {
            ...
            OR_SYM => if matches!(ctx, ExprContext::Assert {negate: true}) {
                children.map(|x| {
                    self.inner.assert(!x.as_bool()?);
                    Ok(())
                }).collect::<Result<(), AddSexpError>>()?;
            ...
            EQ_SYM => {
                ...
                extend_result(&mut c, children.map(|x| {
            ...
            DISTINCT_SYM => {
                ...
                let res = match ctx {
                    ...
                    _ => {
                        let mut c: Conjunction = self.inner.new_junction();
                        c.extend(pairwise_sym(&ids).map(|(id1, id2)| !self.inner.raw_eq(*id1, *id2)));
                        self.inner.collapse_bool(c).into()
                    }
                };
                self.id_buf = ids;
            ...
            ITE_SYM | IF_SYM => {
                ...
                self.inner.ite(i.as_bool()?, t.exp(), e.exp())
            ...
            sym => match self.bound.get(&sym) {
                ...
                Some(Bound::Fn(f)) => {
                    ...
                    if children.len() < f.args.len() {
                        return Err(MissingArgument { actual: children.len(), expected: f.args.len() })
                    }
    ...
    pub fn end_exp(&mut self, independent: bool) -> Result<Exp, (Symbol, AddSexpError)> {
        let Frame{ ctx, f, expected, stack_len } = self.stack.pop().unwrap();
    ...
    pub fn get_definition_values(&mut self) -> (&InternInfo, impl Iterator<Item=(Symbol, BoundDefinition<FunctionAssignment!['_]>)>) {
    ...
    pub fn full_clear(&mut self) {
        ...
        self.bound.insert(TRUE_SYM, Bound::Const(BoolExp::TRUE.into()));
        self.bound.insert(FALSE_SYM, Bound::Const(BoolExp::FALSE.into()));
    }
}
